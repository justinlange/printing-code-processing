class SquareGrid
{
  ...
  SquareGrid(int _cols, int _rows, float _gutterSize, float _pageMargin)
  {
    cols = _cols;
    rows = _rows;
    ...
    rows = floor((actualPageHeight - totalGutterHeight) / (float)moduleWidth );
    println(rows);
  ...
  Module getSquareModule(int numModulesHorizontal, int numModulesVertical)
  {
    counter++;
    ...
    if(numModulesHorizontal != numModulesVertical){
     return null; 
    }
    ...
    if(numModulesVertical != numModulesHorizontal){
      return null;
    }
